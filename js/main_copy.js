...
$(document).ready(function () {
    ...
    app.View.FinishChatMenu = function (filter, counts) {
        ...
        //app.View.renderChatPage('default');
        $('#defaultChat').on("click", function () {
            app.View.renderChatPage('default');
        });
        $('#defaultChatAttendees').on("click", function () {
            app.View.renderAttendeesPage("default");
        });
        ...
        for (var i = 0; i < app.Session.meta.interests.length; i++) {
            ...
            if (app.Session.meta.interests[i] === "art") {
                $('#artChat').on("click", function () {
                    app.View.renderChatPage('art')
                }).removeAttr("disabled", "disabled").removeClass("btn-disabled");
            }

            if (app.Session.meta.interests[i] === "sports") {
                $('#sportsChat').on("click", function () {
                    app.View.renderChatPage('sports')
                }).removeAttr("disabled", "disabled").removeClass("btn-disabled");
            }

            if (app.Session.meta.interests[i] === "music") {
                $('#musicChat').on("click", function () {
                    app.View.renderChatPage('music')
                }).removeAttr("disabled", "disabled").removeClass("btn-disabled");
            }

            if (app.Session.meta.interests[i] === "technology") {
                $('#technologyChat').on("click", function () {
                    app.View.renderChatPage('technology')
                }).removeAttr("disabled", "disabled").removeClass("btn-disabled");
            }

            if (app.Session.meta.interests[i] === "food") {
                $('#foodChat').on("click", function () {
                    app.View.renderChatPage('food')
                }).removeAttr("disabled", "disabled").removeClass("btn-disabled");
            }
        }
        ...
        if (numArtAttendees > 0 && $('#artChat').is(':disabled') == false) {
            $('#artAttendees').removeAttr("disabled", "disabled").removeClass("btn-disabled").click(function () {
                app.View.renderAttendeesPage("art");
            });
        }

        if (numSportsAttendees > 0 && $('#sportsChat').is(':disabled') == false) {
            $('#sportsAttendees').removeAttr("disabled", "disabled").removeClass("btn-disabled").click(function () {
                app.View.renderAttendeesPage("sports");
            })
        }

        if (numMusicAttendees > 0 && $('#musicChat').is(':disabled') == false) {
            $('#musicAttendees').removeAttr("disabled", "disabled").removeClass("btn-disabled").click(function () {
                app.View.renderAttendeesPage("music");
            })
        }

        if (numTechAttendees > 0 && $('#technologyChat').is(':disabled') == false) {
            $('#technologyAttendees').removeAttr("disabled", "disabled").removeClass("btn-disabled").click(function () {
                app.View.renderAttendeesPage("technology");
            })
        }

        if (numFoodAttendees > 0 && $('#foodChat').is(':disabled') == false) {
            $('#foodAttendees').removeAttr("disabled", "disabled").removeClass("btn-disabled").click(function () {
                app.View.renderAttendeesPage("food");
            })
        }
    }
    ...
    app.View.renderChatPage = function (filter) {
        ...
        $('#main_container').html(template({
            filter: tempFilter,
            location: app.Session.event.name
        }));
        $('#back-chatmenu').on("click", function () {
            app.View.renderChatMenu(app.Session.event.name)
        });
        $('#chatbox-submit').on("click", function () {
            app.View.sendChat()
        });
    ...
    app.View.renderAttendeesPage = function (filter) {
        ...
        $('#main_container').html('');
        $('#main_container').html(template());
        $('#back-attendee-chatmenu').on("click", function () {
            app.View.renderChatMenu(app.Session.event.name)
        });
